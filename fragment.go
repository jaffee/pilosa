...
func (f *fragment) reopen() (err error, mustClose bool) {
	if f.file == nil {
		// Open the data file to be mmap'd and used as an ops log.
		f.file, err, mustClose = syswrap.OpenFile(f.path, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
		if err != nil {
			return fmt.Errorf("open file: %s", err), mustClose
		}
		f.storage.OpWriter = f.file
	}
	return nil, mustClose
}
...
func (f *fragment) openStorage() error {
	...
	// Open the data file to be mmap'd and used as an ops log.
	file, err, mustClose := syswrap.OpenFile(f.path, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
